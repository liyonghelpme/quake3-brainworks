...
qboolean BotSetAimType(bot_state_t *bs, int type, gentity_t *ent, vec3_t loc)
{
	...
	if (bs->debug_flags & BOT_DEBUG_INFO_AIM)
	{
		...
		switch (type)
		{
			case AIM_ACTIVATOR:		target = "shot activated button"; 	break;
			case AIM_JUMP:			target = "jump direction";			break;
			case AIM_ENEMY:			target = "enemy";					break;
			case AIM_KAMIKAZE:		target = "kamikaze body";			break;
			case AIM_MINE:			target = "mine";					break;
			case AIM_MAPOBJECT:		target = "map object";				break;
			case AIM_SWIM:			target = "swim direction";			break;
			case AIM_FACEENTITY:	target = "facing entity";			break;
			case AIM_MOVEMENT:		target = "movement aim hint";		break;
			case AIM_AWARE:			target = "awareness trigger";		break;
			case AIM_STRAFEJUMP:	target = "strafe jumping";			break;
			case AIM_GOAL:			target = "goal";					break;
			case AIM_SEARCH:		target = "searching";				break;
...
int BotAimLocChangeDetect(bot_state_t *bs, vec3_t aim_loc)
{
	...
	for (i = PITCH; i <= YAW; i++)
	{
		...
		// Otherwise compute the speed differentially
			...
			new_speeds[i] =
			  AngleDelta(aim_angles[i], bs->view_ideal_next[i].angle.real)
				/ time_change;
	}
...
============
*/
void BotSetAimLoc(bot_state_t *bs, int aim_type, gentity_t *aim_ent,
				  vec3_t aim_loc, vec3_t aim_speed, vec3_t ref_loc)
{
	int i, changes;
	float dist, inv_time;
	...
	if (aim_speed)
	{
		...
		for (i = PITCH; i <= ROLL; i++)
			aim_angle_speeds[i] = AngleDelta(next_angles[i], aim_angles[i]) *
								  SERVER_FRAMES_PER_SEC;
	}
	else
	{
...
qboolean BotAimTarget(bot_state_t *bs, int aim_type, gentity_t *ent, int weapon)
{
	...
	if ( (bs->teleport_time > 0) &&
		 (bs->command_time < bs->react_time + bs->teleport_time ) )
	{
		return qfalse;
	}
	...
	BotSetAimLoc(bs, aim_type, bs->attack.ent,
				 shot_loc, bs->attack.motion.velocity,
				 bs->attack.reference);
...
qboolean BotAimActivator(bot_state_t *bs, bot_path_t *path)
{
	...
	return BotAimTarget(bs, AIM_ACTIVATOR, &g_entities[path->subgoal->entitynum],
						BotActivateWeapon(bs));
}
...
qboolean BotAimMap(bot_state_t *bs)
{
	...
	if ( (bs->now.origin[0] > mins[0] && bs->now.origin[0] < maxs[0]) &&
		 (bs->now.origin[1] > mins[1] && bs->now.origin[1] < maxs[1]) &&
		 (bs->now.origin[2] < mins[2]) )
	{
		return qfalse;
	}
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++)
	{
		...
		if ( (ent->r.currentOrigin[0] > mins[0] && ent->r.currentOrigin[0] < maxs[0]) &&
			 (ent->r.currentOrigin[1] > mins[1] && ent->r.currentOrigin[1] < maxs[1]) &&
			 (ent->r.currentOrigin[2] < mins[2]) )
		{
...
float BotSelectSearch(bot_state_t *bs, vec3_t target)
{
	...
	for (i = 0; i < SEARCH_POINTS; i++)
	{
		...
		VectorSet(point,
				  ...
				  trace.endpos[1] * .95 + bs->eye_future[1] * .05,
				  trace.endpos[2] * .95 + bs->eye_future[2] * .05);
		...
		trace.endpos[2] += 1.0;
		if ( trap_PointContents(trace.endpos, bs->entitynum) &
			 (CONTENTS_NODROP|CONTENTS_LAVA|CONTENTS_SLIME) )
		{
			continue;
		}
...
qboolean BotAimSearch(bot_state_t *bs)
{
	...
	if (new_target)
	{
		...
		bs->search_timeout = bs->command_time
							 + MIN_SEARCH_TIME
							 + (MAX_SEARCH_TIME - MIN_SEARCH_TIME) * random();
	}
...
void BotAimSelect(bot_state_t *bs, bot_moveresult_t *moveresult)
{
	...
	if ( !BotAimActivator(bs, &bs->item_path)	&&
		 !BotAimActivator(bs, &bs->main_path)	&&
		 !BotAimJump(bs)						&&
		 !BotAimMap(bs)							&&
		 !BotAimEnemy(bs)						&&
#ifdef MISSIONPACK
		 !BotAimKamikaze(bs)					&&
		 !BotAimMine(bs, moveresult)			&&
#endif
