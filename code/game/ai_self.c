...
qboolean BotIsCarrier(bot_state_t *bs)
{
	switch (gametype)
	{
		case GT_CTF:		return (bs->ps->powerups[PW_REDFLAG] ||
									bs->ps->powerups[PW_BLUEFLAG]);
#ifdef MISSIONPACK
		case GT_1FCTF:		return (bs->ps->powerups[PW_NEUTRALFLAG]);
		case GT_HARVESTER:	return (bs->ps->generic1 > 0);
#endif
	}
...
int BotTeamBase(bot_state_t *bs)
{
	...
	switch(BotTeam(bs)) {
		case TEAM_RED:	return RED_BASE;
		case TEAM_BLUE:	return BLUE_BASE;
		default:		return -1;
	}
}
...
int BotCaptureBase(bot_state_t *bs)
{
	...
	switch (gametype)
	{
		...
		case GT_CTF:
			if (bs->ps->powerups[PW_REDFLAG])		return BLUE_BASE;
			if (bs->ps->powerups[PW_BLUEFLAG])		return RED_BASE;
			break;

#ifdef MISSIONPACK
		// Capture the flag at the enemy base
		case GT_1FCTF:
			if (bs->ps->powerups[PW_NEUTRALFLAG])	return BotEnemyBase(bs);
			break;

		// Capture heads at the enemy base
		case GT_HARVESTER:
			if (bs->ps->generic1 > 0)				return BotEnemyBase(bs);
			break;
#endif
	}
...
void BotSetInfoConfigString(bot_state_t *bs)
{
	...
	{
		case ORDER_ATTACK:		action = "attacking";
								ent = bs->order_enemy;					break;
		case ORDER_HELP:		action = "helping";
								ent = bs->help_teammate;				break;
		case ORDER_ACCOMPANY:	action = "accompanying";
								ent = bs->accompany_teammate;			break;

		case ORDER_DEFEND:		action = "defending";
								goal = &bs->defend_goal;				break;
		case ORDER_ITEM:		action = "getting item";
								goal = &bs->inspect_goal;				break;

		case ORDER_GETFLAG:		action = "getting the flag";			break;
		case ORDER_RETURNFLAG:	action = "returning the flag";			break;
		case ORDER_HARVEST:		action = "harvesting";					break;
		case ORDER_ASSAULT:		action = "assaulting the enemy base";	break;
		case ORDER_CAMP:		action = "camping";						break;
		case ORDER_PATROL:		action = "patrolling";					break;

		default:
		case ORDER_NONE:		action = "roaming";						break;
	}

	if (ent)
		cs = va("l\\%s\\c\\%s\\a\\%s %s",
				leader,	carrying, action, SimplifyName(EntityNameFast(ent)));
	else if (goal)
		cs = va("l\\%s\\c\\%s\\a\\%s %s",
				leader,	carrying, action, GoalNameFast(goal));
	else
		cs = va("l\\%s\\c\\%s\\a\\%s",
				leader,	carrying, action);
