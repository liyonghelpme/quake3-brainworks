...
qboolean BotSafeChatPosition(bot_state_t *bs)
{
	...
	if (bs->ps->powerups[PW_QUAD] ||
		...
		bs->ps->powerups[PW_INVIS] ||
		bs->ps->powerups[PW_REGEN] ||
		bs->ps->powerups[PW_FLIGHT] ||
...
qboolean BotChatEnterGame(bot_state_t *bs)
{
	...
	if ( (!bot_fastchat.integer) &&
		 (random() > trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_ENTEREXITGAME, 0, 1)) )
	{
		return qfalse;
	}

	Bot_InitialChat(bs, "game_enter",
					SimplifyName(EntityNameFast(bs->ent)),	// 0
...
qboolean BotChatStartLevel(bot_state_t *bs)
{
	...
	Bot_InitialChat(bs, "level_start",
					SimplifyName(EntityNameFast(bs->ent)),		// 0
					NULL);
...
void BotChatEndLevel(bot_state_t *bs)
{
	...
	if (BotIsFirstInRankings(bs)) {
		Bot_InitialChat(bs, "level_end_victory",
						SimplifyName(EntityNameFast(bs->ent)),	// 0
						BotRandomOpponentName(bs),				// 1
						"[invalid var]",						// 2
		...
		Bot_InitialChat(bs, "level_end_lose",
						SimplifyName(EntityNameFast(bs->ent)),	// 0
						BotRandomOpponentName(bs),				// 1
	...
	else
	{
		if (bs->bot_death_type == MOD_WATER)
			...
			Bot_InitialChat(bs, "death_cratered", BotRandomOpponentName(bs), NULL);
				...
				bs->bot_death_type == MOD_TRIGGER_HURT ||
				bs->bot_death_type == MOD_UNKNOWN)
			...
			if ((bs->bot_death_type == MOD_GAUNTLET ||
				...
				bs->bot_death_type == MOD_BFG ||
				bs->bot_death_type == MOD_BFG_SPLASH) && random() < 0.5) {

				if (bs->bot_death_type == MOD_GAUNTLET)
					Bot_InitialChat(bs, "death_gauntlet",
									name,												// 0
									WeaponNameForMeansOfDeath(bs->bot_death_type),		// 1
									NULL);
				else if (bs->bot_death_type == MOD_RAILGUN)
					Bot_InitialChat(bs, "death_rail",
									name,												// 0
									WeaponNameForMeansOfDeath(bs->bot_death_type),		// 1
									NULL);
				else
					Bot_InitialChat(bs, "death_bfg",
									name,												// 0
									WeaponNameForMeansOfDeath(bs->bot_death_type),		// 1
									NULL);
			}
			//choose between insult and praise
				...
				Bot_InitialChat(bs, "death_insult",
								name,												// 0
								WeaponNameForMeansOfDeath(bs->bot_death_type),		// 1
								NULL);
			}
			else {
				Bot_InitialChat(bs, "death_praise",
								name,												// 0
								WeaponNameForMeansOfDeath(bs->bot_death_type),		// 1
								NULL);
			}
...
void BotChatHitTalking(bot_state_t *bs)
{
	...
	Bot_InitialChat(bs, "hit_talking",
						 ...
						 WeaponNameForMeansOfDeath(bs->ent->client->lasthurt_mod),
						 NULL);
...
qboolean BotWantsKillChat(bot_state_t *bs)
{
	...
	// The bot might not want to chat all the time
	if ( (!bot_fastchat.integer) &&
		 (random() > trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_KILL, 0, 1)) )
	{
		return qfalse;
	}
...
qboolean BotChatKill(bot_state_t *bs)
{
	...
	{
		case MOD_GAUNTLET:	type = "kill_gauntlet";	break;
		case MOD_RAILGUN:	type = "kill_rail";		break;
		case MOD_TELEFRAG:	type = "kill_telefrag";	break;

		default:
#ifdef MISSIONPACK
			if (bs->killed_player_type == MOD_KAMIKAZE &&
...
qboolean BotChatHitNoDeath(bot_state_t *bs)
{
	...
	// Don't always give messages at every opportunity
	if ( (!bot_fastchat.integer) &&
		 (random() > trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CHAT_HITNODEATH, 0, 1)) )
	{
		return qfalse;
	}
	...
	Bot_InitialChat(bs, "hit_nodeath",
					EntityNameFast(bs->last_hurt_client),
					WeaponNameForMeansOfDeath(bs->ent->client->lasthurt_mod),
					NULL);
	Bot_SetupChatInfo(bs, BotChatTime(bs), CHAT_ALL, 0);
...
void BotChatIngame(bot_state_t *bs)
{
	...
	if ( !(game_style & GS_TEAM) )
	{
		...
		if ( BotWantsKillChat(bs) 		 ||
			 BotWantsStartLevelChat(bs) ||
			 BotWantsRandomChat(bs) )
			trap_EA_Command(bs->client, "vtaunt");
	}
...
void BotCheckConsoleMessages(bot_state_t *bs)
{
	...
	for (handle = trap_BotNextConsoleMessage(bs->cs, &m);
		 ...
		 handle = trap_BotNextConsoleMessage(bs->cs, &m))
	{
		...
		if (trap_BotNumConsoleMessages(bs->cs) < 10)
		{
			...
			if ( (m.type == CMS_CHAT) &&
				 (m.time > bs->command_time - (1.0 + random())) )
				break;
		}
...
void BotChatTest(bot_state_t *bs)
{
	...
	for (i = 0; i < num; i++)
	{
		Bot_InitialChat(bs, "random_misc",
					BotRandomOpponentName(bs),	// 0
					enemy_name,					// 1
					"[invalid var]",			// 2
					"[invalid var]",			// 3
	...
	for (i = 0; i < num; i++)
	{
		Bot_InitialChat(bs, "random_insult",
					BotRandomOpponentName(bs),	// 0
					enemy_name,					// 1
					"[invalid var]",			// 2
					"[invalid var]",			// 3
