...
==============
*/
void FrameOptionAdd(item_frame_t *frame, item_cluster_t **chain, int chain_size,
					float rating, qboolean selected)
{
	int index;
	qboolean insert;
	...
	option = bsearch_ins(&cluster, frame->option, &frame->num_options,
						 MAX_CLUSTERS_CONSIDER, sizeof(chain_option_t),
						 CompareClusterChainOption, &insert);
...
void FrameOptionPrint(item_frame_t *frame)
{
	...
	G_Printf("  %.3f: ^3%s^7 (bot nearest ^2%s^7)\n",
			 frame->time, ClusterName(cluster),
			 RegionName(LevelNearestRegion(frame->loc)));
...
void BotPrintItemTrack(tvl_t *tvl, int index, void *bs)
{
	...
	BotAI_Print(PRT_MESSAGE, "%s: Timed Item: Tracking %s\n",
				EntityNameFast( ((bot_state_t *) bs)->ent ),
				ClusterName(cluster));
}
...
qboolean BotTimeCluster(bot_state_t *bs, item_cluster_t *cluster)
{
	...
	if (bs->debug_flags & BOT_DEBUG_INFO_TIMED_ITEM)
		tracked = tvl_add(&bs->timed_items, &cluster, respawn_time, cluster->value,
						  BotPrintItemTrack, BotPrintItemLoss, bs)
					>= 0;
	else
#endif
		tracked = tvl_add(&bs->timed_items, &cluster, respawn_time, cluster->value,
...
=========================
*/
qboolean BotItemClusterOptionSetup(bot_state_t *bs, cluster_option_t *option,
...

{
	...
	option->from_start = LevelTravelTime(info->area, bs->now.origin,
										 cluster_area, cluster_loc, bs->travel_flags);
	...
	if (info->time >= 0.0)
	{
		...
		option->to_goal = LevelTravelTime(cluster_area, cluster_loc,
				bs->goal.areanum, bs->goal.origin, bs->travel_flags);
	...
	option->selected = (bs->num_item_clusters &&
						option->cluster == bs->item_clusters[0] &&
						option->cluster->center->ent == bs->item_centers[0]);
...
void BotItemClusterSelectSetup(bot_state_t *bs, bot_item_info_t *info)
{
	...
	info->start_region =
		BotEncounterRateLoc(bs, bs->ps->origin, qtrue, info->teammates, info->enemies,
							&info->start_rates);

	// Determine how long it will take to reach the goal in seconds
	info->time = LevelTravelTime(info->area, bs->now.origin,
								 bs->goal.areanum, bs->goal.origin, bs->travel_flags);
...
void BotItemClusterOptionsSetup(bot_state_t *bs, bot_item_info_t *info)
{
	...
	for (i = 0; i < info->num_consider; i++)
	{
		...
		if (ignored > 0)
			memcpy(&info->consider[i - ignored],
				   &info->consider[i],
				   sizeof(cluster_option_t));
	}
...
float BotNoPickupConsider(bot_state_t *bs, bot_item_info_t *info)
{
	...
	if (info->time >= 0.0)
	{
		...
		ResourcePredictEncounter(&rs, second_predict_time, 1.0,
								 info->end_rates.see_enemy, info->end_rates.see_enemy);
	}
	else
	{
...
void BotItemClusterSetConsider(bot_state_t *bs, bot_item_info_t *info)
{
	...
	while (options.valid)
	{
		...
		if (last_option &&
 			!LevelRegionIsNeighbor(this_option->cluster->region,
 								   last_option->neighbors,
 								   last_option->num_neighbors))
		{
			isi_skip(&options);
			continue;
		}
		...
		ResourcePredictEncounter(this_rs, time * .5, initial_score,
								 last_rates->see_enemy, last_rates->enemy_attack);
		ResourcePredictEncounter(this_rs, time * .5, 1.0,
								 this_option->rates.see_enemy, this_option->rates.enemy_attack);
		...
		else
		{
			...
			ResourcePredictEncounter(&end_rs, time, 1.0,
									 info->end_rates.see_enemy,
									 info->end_rates.enemy_attack);
		}
...
void BotItemGoal(bot_state_t *bs)
{
	...
	if (!item)
	{
		...
		if ( (bs->debug_flags & BOT_DEBUG_INFO_ITEM) &&
			 (last_cluster_item != bs->item_centers[0] || last_item != bs->item_ent) )
	...
	if ( (bs->debug_flags & BOT_DEBUG_INFO_ITEM) &&
		...
		BotAI_Print(PRT_MESSAGE, "%s: Item: Cluster: %s, Item: %s (%.f, %.f, %.f)\n",
					...
					bs->item_ent->r.currentOrigin[0],
					bs->item_ent->r.currentOrigin[1],
					bs->item_ent->r.currentOrigin[2]);
#endif
