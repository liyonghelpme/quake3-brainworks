...
int bsearch_addr(register const void *key, const void *list,
				 size_t list_size, register size_t entry_size,
				 register cmp_t *compare, register void **match)
{
...
void *bsearch_ins(const void *key, void *list,
				  size_t *list_size, size_t max_list_size, size_t entry_size,
...
void *tvl_highest_value(tvl_t *tvl, void *prefer)
{
	...
	for (i = 0; i < tvl->size; i++)
	{
		...
		if ( !(best_entry) ||
			 (tvl->value[i] > highest_value) ||
			 (tvl->value[i] == highest_value && !tvl->compare(prefer, entry)) )
		{
			best_entry    = entry;
...
===============
*/
octree_node_t *octree_assemble(octree_node_t *nodes, int num_nodes,
							   const float *(data_location)(const void *))
{
	...
	for (front_sector = 0, back_sector = 7-front_sector;
		 ...
		 front_sector++, back_sector = 7-front_sector)
	{
...
void octree_neighbor_recurse(vec3_t point, octree_node_t *root,
							 ...
							 int *num_neighbors, int *furthest_neighbor)
{
	...
	if (*num_neighbors < max_neighbors)
	{
		...
		if ( (*furthest_neighbor < 0) ||
			 (dists[*furthest_neighbor] < dists[i]) )
			*furthest_neighbor = i;
	}

	// Also track it as a neighbor if it's nearer than the furthest neighbor
	else if ( (*furthest_neighbor >= 0) &&
			  (sector_dist[OT_XYZ] < dists[*furthest_neighbor]) )
	{
	...
	order_code = ((sector_dist[OT_X] < sector_dist[OT_Y]) << 0) |
			 	 ((sector_dist[OT_Y] < sector_dist[OT_Z]) << 1) |
			 	 ((sector_dist[OT_Z] < sector_dist[OT_X]) << 2);
	...
	order_code |= ( sector_dist[ sector_order[order_code][1] ] +
					sector_dist[ sector_order[order_code][2] ]
						<
					sector_dist[ sector_order[order_code][3] ] )
				  << 3;
	...
	for (i = 0; i < 8; i++)
	{
		...
		if ( (max_neighbors <= *num_neighbors) &&
			 (*furthest_neighbor > 0) &&
			 (dists[*furthest_neighbor] <= sector_dist[sector_modify]) )
			break;
		...
		octree_neighbor_recurse(point, root->sector[subsector], data_location,
								max_neighbors, neighbors, dists,
								num_neighbors, furthest_neighbor);
	}
}
					 ...
					 int max_neighbors, void *neighbors[], float dists[])
{
	...
	num_neighbors = 0;
	furthest_neighbor = -1;
	octree_neighbor_recurse(point, root, data_location, max_neighbors,
							neighbors, dists,
							&num_neighbors, &furthest_neighbor);
