...
float LevelTravelTime(int start_area, vec3_t start_loc, int end_area, vec3_t end_loc, int tfl)
{
	...
	if (time < 2.0)
	{
		...
		trap_AAS_PredictRoute(&route, start_area, start_loc, end_area, tfl,
							  32, time * 100 * 10, 0, 0, 0, 0);
		...
		if ( !(route.stopevent & RSE_NOROUTE) && (route.endarea == end_area) )
			time += Distance(route.endpos, end_loc) /
					(g_speed.value > 0 ? g_speed.value : 320.0);
	}
...
qboolean LevelAreasNearby(int start_area, vec3_t start_origin, int end_area)
{
	...
	return ( (!route.stopevent) &&
			 (route.numareas < NEARBY_AREAS) &&
			 (route.time < NEARBY_TIME) );
}
...
{
	if (from_base < 0 || from_base >= NUM_BASES ||
		  to_base < 0 ||   to_base >= NUM_BASES)
		return -1;
...
void LevelBaseSetup( void )
{
	...
	for (ent = &g_entities[0]; ent < &g_entities[level.num_entities]; ent++)
	{
		...
		for (i = 0; i < NUM_BASES; i++)
		{
			...
			if (!Q_stricmp(ent->classname, spawn_name[i]) &&
				GoalEntity(&bases[i], ent))
			{
				base_ents[i] = ent;
				break;
			}
	...
	for (i = 0; i < NUM_BASES; i++)
	{
		for (j = 0; j < NUM_BASES; j++)
		{
			...
			base_travel_time[i][j] =
			  LevelTravelTime(bases[i].areanum, bases[i].origin,
			  				  bases[j].areanum, bases[j].origin, TFL_DEFAULT);
		}
	}
}
...
gentity_t *LevelFindFlag(gentity_t *last_flag, gentity_t *base, char *classname, int powerup)
{
	...
	if (last_flag && last_flag->inuse)
	{
		if ( (last_flag->s.eType == ET_ITEM) &&
			 (last_flag->r.contents == CONTENTS_TRIGGER) &&
			 (!Q_stricmp(last_flag->classname, classname)) )
			return last_flag;
		if ( (last_flag->client) &&
			 (last_flag->client->ps.powerups[powerup]) )
			return last_flag;
	}
	...
	while (ent < &g_entities[level.num_entities])
	{
		if ( (ent->s.eType == ET_ITEM) &&
			 (ent->flags & FL_DROPPED_ITEM) &&
			 (ent->r.contents == CONTENTS_TRIGGER) &&
			 (!Q_stricmp(ent->classname, classname)) )
		{
			return ent;
		}
...
void LevelFlagScan(void)
{
	...
	{
		case GT_CTF:
			flags[RED_BASE ] = LevelFindFlag(flags[RED_BASE ], base_ents[RED_BASE ],
											 "team_CTF_redflag",  PW_REDFLAG);
			flags[BLUE_BASE] = LevelFindFlag(flags[BLUE_BASE], base_ents[BLUE_BASE],
											 "team_CTF_blueflag", PW_BLUEFLAG);
			break;

#ifdef MISSIONPACK
		case GT_1FCTF:
			flags[MID_BASE ] = LevelFindFlag(flags[MID_BASE ], base_ents[MID_BASE ],
											 "team_CTF_neutralflag", PW_NEUTRALFLAG);
			break;
#endif
	}
}
