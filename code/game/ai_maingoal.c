...
void BotSetGoalType(bot_state_t *bs, int type)
{
	...
	if ( (bs->goal_type   == type) &&
		 ...
		 (bs->goal_area   == bs->goal.areanum || bs->goal.entitynum >= 0) )
		return;
	...
	switch (type)
	{
		case GOAL_AIR:					bs->goal_value = GOAL_VALUE_CRITICAL;	break;
		case GOAL_LEAD:					bs->goal_value = GOAL_VALUE_MEDIUM;		break;
		case GOAL_CAPTURE:				bs->goal_value = GOAL_VALUE_HIGH;		break;
		case GOAL_CAPTURE_WAIT:			bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_ATTACK_CHOICE:		bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_ATTACK_ORDER:			bs->goal_value = GOAL_VALUE_MEDIUM;		break;
		case GOAL_HELP_CHOICE:			bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_HELP_ORDER:			bs->goal_value = GOAL_VALUE_MEDIUM;		break;
		case GOAL_ACCOMPANY_CHOICE:		bs->goal_value = GOAL_VALUE_MEDIUM;		break;
		case GOAL_ACCOMPANY_ORDER:		bs->goal_value = GOAL_VALUE_HIGH;		break;
		case GOAL_DEFEND_CHOICE:		bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_DEFEND_ORDER:			bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_PATROL:				bs->goal_value = GOAL_VALUE_VERYLOW;	break;
		case GOAL_INSPECT_CHOICE:		bs->goal_value = GOAL_VALUE_VERYLOW;	break;
		case GOAL_INSPECT_ORDER:		bs->goal_value = GOAL_VALUE_MEDIUM;		break;
		case GOAL_CAMP_CHOICE:			bs->goal_value = GOAL_VALUE_VERYLOW;	break;
		case GOAL_CAMP_ORDER:			bs->goal_value = GOAL_VALUE_VERYLOW;	break;
		case GOAL_GETFLAG_CHOICE:		bs->goal_value = GOAL_VALUE_HIGH;		break;
		case GOAL_GETFLAG_ORDER:		bs->goal_value = GOAL_VALUE_HIGH;		break;
		case GOAL_RETURNFLAG_CHOICE:	bs->goal_value = GOAL_VALUE_CRITICAL;	break;
		case GOAL_RETURNFLAG_ORDER:		bs->goal_value = GOAL_VALUE_CRITICAL;	break;
		case GOAL_ASSAULT_CHOICE:		bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_ASSAULT_ORDER:		bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_HARVEST_CHOICE:		bs->goal_value = GOAL_VALUE_LOW;		break;
		case GOAL_HARVEST_ORDER:		bs->goal_value = GOAL_VALUE_LOW;		break;

		default:
		case GOAL_NONE:					bs->goal_value = GOAL_VALUE_NONE;		break;
	}
	...
	switch (type)
	{
		case GOAL_NONE:					action = "Nothing";				break;
		case GOAL_AIR:					action = "Air";					break;
		case GOAL_LEAD:					action = "Lead";				break;
		case GOAL_CAPTURE:				action = "Capture";				break;
		case GOAL_CAPTURE_WAIT:			action = "Waiting to Capture";	break;
		case GOAL_ATTACK_CHOICE:		action = "Attack choice";		break;
		case GOAL_ATTACK_ORDER:			action = "Attack order";		break;
		case GOAL_HELP_CHOICE:			action = "Help choice";			break;
		case GOAL_HELP_ORDER:			action = "Help order";			break;
		case GOAL_ACCOMPANY_CHOICE:		action = "Accompany choice";	break;
		case GOAL_ACCOMPANY_ORDER:		action = "Accompany order";		break;
		case GOAL_DEFEND_CHOICE:		action = "Defend choice";		break;
		case GOAL_DEFEND_ORDER:			action = "Defend order";		break;
		case GOAL_PATROL:				action = "Patrol";				break;
		case GOAL_INSPECT_CHOICE:		action = "Inspect choice";		break;
		case GOAL_INSPECT_ORDER:		action = "Inspect order";		break;
		case GOAL_CAMP_CHOICE:			action = "Camp choice";			break;
		case GOAL_CAMP_ORDER:			action = "Camp order";			break;
		case GOAL_GETFLAG_CHOICE:		action = "Get flag choice";		break;
		case GOAL_GETFLAG_ORDER:		action = "Get flag order";		break;
		case GOAL_RETURNFLAG_CHOICE:	action = "Return flag choice";	break;
		case GOAL_RETURNFLAG_ORDER:		action = "Return flag order";	break;
		case GOAL_ASSAULT_CHOICE:		action = "Assault choice";		break;
		case GOAL_ASSAULT_ORDER:		action = "Assault order";		break;
		case GOAL_HARVEST_CHOICE:		action = "Harvest choice";		break;
		case GOAL_HARVEST_ORDER:		action = "Harvest order";		break;
...
int BotGoalCaptureChoice(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( (gametype == GT_CTF) &&
		 (!BotTeammates(bs)) &&
		 (bs->their_target_flag_status != FS_AT_HOME) )
		return GOAL_NONE;
	...
	if (gametype == GT_HARVESTER)
	{
		...
		if ( (num_heads)   * (time_to_mid + head_respawn_rate * .5 + time_from_mid_to_base) <
			 (num_heads+1) * (time_to_base) )
			return GOAL_NONE;
	}
#endif
	...
	if ( (gametype == GT_CTF) &&
		 (bs->their_target_flag_status != FS_AT_HOME) &&
		 (DistanceSquared(bs->now.origin, goal->origin) < Square (1024.0)) )
...
int BotGoalAttackOrder(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if (bs->killed_player == bs->order_enemy ||
		bs->order_enemy->client->ps.pm_type == PM_DEAD)
	{
		BotOrderAnnounceReset(bs, "kill_done", bs->order_requester,
							  SimplifyName(EntityNameFast(bs->order_enemy)));
		return GOAL_NONE;
	}
...
int BotGoalHelpOrder(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( !bs->help_teammate ||
		 bs->order_time < bs->command_time ||
		 !BotHelpPlayer(bs, goal) )
	{
...
int BotGoalAccompanyOrder(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if (!goal->areanum && bs->announce_arrive)
	{
		...
		Bot_InitialChat(bs, "accompany_arrive",
						SimplifyName(EntityNameFast(bs->accompany_teammate)), NULL);
...
int BotDefendLocation(bot_state_t *bs, bot_goal_t *goal, bot_goal_t *location, int type)
{
	...
	if ( bs->goal_enemy &&
		 DistanceSquared(bs->goal_enemy->r.currentOrigin, location->origin) < Square(1280.0) )
	{
...
int BotGoalDefendOrder(bot_state_t *bs, bot_goal_t *goal)
{
	...
	// Possibly announce start of order acceptance
	if (BotOrderShouldAnnounce(bs))
		BotOrderAnnounceStart(bs, "defend_start", bs->order_requester,
							  GoalNameFast(&bs->defend_goal), VOICECHAT_YES);
	return type;
}
...
qboolean BotChooseItemInspect(bot_state_t *bs, item_cluster_t *exclude)
{
	...
	if (exclude)
	{
		...
		excluded_entry =
		  bsearch(&exclude, items, num_items, sizeof(item_cluster_t *), CompareVoidList);
...
int BotGoalItemChoice(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( (bs->inspect_cluster) &&
		 (bs->command_time - 3.0 < bs->inspect_time_last) )
	{
		...
		else if (DistanceSquared(bs->inspect_cluster->center->ent->r.currentOrigin,
								 bs->now.origin) <
				 Square(128.0))
		{
...
int BotGoalItemOrder(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( (bs->order_time < bs->command_time) ||
		 !BotItemInspect(bs, goal, &bs->inspect_goal) )
		return GOAL_NONE;

	// Finish the order if the bot reached the item pickup location
	if (trap_BotTouchingGoal(bs->now.origin, &bs->inspect_goal))
	{
		BotOrderAnnounceReset(bs, "getitem_gotit", bs->order_requester,
							  GoalNameFast(&bs->inspect_goal));
		return GOAL_NONE;
	}
	...
	if ( !(item->r.contents & CONTENTS_TRIGGER) &&
		 BotTargetInFieldOfVision(bs, item->r.currentOrigin, 90) &&
		 BotEntityVisibleFast(bs, item) )
	{
		BotOrderAnnounceReset(bs, "getitem_notthere", bs->order_requester,
							  GoalNameFast(&bs->inspect_goal));
		return GOAL_NONE;
	}
	...
	if (BotOrderShouldAnnounce(bs))
		BotOrderAnnounceStart(bs, "getitem_start", bs->order_requester,
							  GoalNameFast(&bs->inspect_goal), VOICECHAT_YES);
...
qboolean BotChooseCampLocation(bot_state_t *bs, float max_time)
{
	...
	for (camp_spot_id = trap_BotGetNextCampSpotGoal(0, &goal);
		 camp_spot_id;
		 camp_spot_id = trap_BotGetNextCampSpotGoal(camp_spot_id, &goal))
	{
...
int BotGoalCampChoice(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( (bs->last_camp_time < bs->command_time - 3.0) ||
		 (bs->end_camp_time <= bs->command_time) )
	{
		...
		if ( (bs->camp_goal.areanum) ||
			 (camper < random()) ||
			 (!BotChooseCampLocation(bs, 5)) )
		{
...
int BotGoalCampOrder(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( (bs->order_time < bs->command_time) ||
		 (!BotCampLocation(bs, goal, &bs->camp_goal)) )
	{
...
qboolean BotGetFlag(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if ( bs->our_target_flag_status != FS_AT_HOME &&
		 bs->our_target_flag_status != FS_DROPPED )
		return qfalse;
...
void BotComputeGoalCheckList(bot_state_t *bs)
{
	...
	if ( (bs->goal_sieve_valid) &&
		 (!bs->goal_sieve_recompute_time || bs->goal_sieve_recompute_time > bs->command_time) )
	{
		return;
	}
	...
	if (game_style & GS_TEAM)
	{
		...
		{
			case ORDER_HELP:		BotAddGoalCheck(bs, BotGoalHelpOrder);			break;
			case ORDER_ACCOMPANY:	BotAddGoalCheck(bs, BotGoalAccompanyOrder);		break;
			case ORDER_ITEM:		BotAddGoalCheck(bs, BotGoalItemOrder);			break;
			case ORDER_ATTACK:		BotAddGoalCheck(bs, BotGoalAttackOrder);		break;
			case ORDER_GETFLAG:		BotAddGoalCheck(bs, BotGoalGetFlagOrder);		break;
			case ORDER_RETURNFLAG:	BotAddGoalCheck(bs, BotGoalReturnFlagOrder);	break;
			case ORDER_DEFEND:		BotAddGoalCheck(bs, BotGoalDefendOrder);		break;
			case ORDER_CAMP:		BotAddGoalCheck(bs, BotGoalCampOrder);			break;
			case ORDER_PATROL:		BotAddGoalCheck(bs, BotGoalPatrolOrder);		break;
			case ORDER_ASSAULT:		BotAddGoalCheck(bs, BotGoalAssaultOrder);		break;
#ifdef MISSIONPACK
			case ORDER_HARVEST:		BotAddGoalCheck(bs, BotGoalHarvestOrder);		break;
#endif
		}
	}
	...
	switch (gametype)
	{
		case GT_CTF:		BotAddChoicesCTF(bs);		break;
#ifdef MISSIONPACK
		case GT_1FCTF:		BotAddChoices1FCTF(bs);		break;
		case GT_HARVESTER:	BotAddChoicesHarvester(bs);	break;
		case GT_OBELISK:	BotAddChoicesObelisk(bs);	break;
#endif
	}
