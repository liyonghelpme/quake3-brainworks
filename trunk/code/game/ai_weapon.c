...
char *WeaponNameForMeansOfDeath(int means_of_death)
{
	...
	{
		case MOD_SHOTGUN:			return WeaponName(WP_SHOTGUN);
		case MOD_GAUNTLET:			return WeaponName(WP_GAUNTLET);
		case MOD_MACHINEGUN:		return WeaponName(WP_MACHINEGUN);
		case MOD_GRENADE:
		case MOD_GRENADE_SPLASH:	return WeaponName(WP_GRENADE_LAUNCHER);
		case MOD_ROCKET:
		case MOD_ROCKET_SPLASH:		return WeaponName(WP_ROCKET_LAUNCHER);
		case MOD_PLASMA:
		case MOD_PLASMA_SPLASH:		return WeaponName(WP_PLASMAGUN);
		case MOD_RAILGUN:			return WeaponName(WP_RAILGUN);
		case MOD_LIGHTNING:			return WeaponName(WP_LIGHTNING);
		case MOD_BFG:
		case MOD_BFG_SPLASH:		return WeaponName(WP_BFG);
#ifdef MISSIONPACK
		case MOD_NAIL:				return WeaponName(WP_NAILGUN);
		case MOD_CHAINGUN:			return WeaponName(WP_CHAINGUN);
		case MOD_PROXIMITY_MINE:	return WeaponName(WP_PROX_LAUNCHER);
		case MOD_KAMIKAZE:			return "Kamikaze";
		case MOD_JUICED:			return "Prox mine";
#endif
		case MOD_GRAPPLE:			return WeaponName(WP_GRAPPLING_HOOK);

		// Force consistant error message
		default:					return WeaponName(-1);
	}
}
...
int WeaponFromName(char *name)
{
	...
	if (num_weapon_aliases < 0)
	{
		...
		qsort(weapon_aliases, num_weapon_aliases, sizeof(entry_string_int_t),
			  CompareEntryStringInsensitive);
	}

	// Search for a matching name
	alias = bsearch(name, weapon_aliases, num_weapon_aliases, sizeof(entry_string_int_t),
...
{
	return (bs->ps->weaponstate == WEAPON_DROPPING ||
			bs->ps->weaponstate == WEAPON_RAISING);
}
...
{
	return ( (bs->ps->stats[STAT_WEAPONS] & (1 << weapon)) &&
			 (bs->ps->ammo[weapon] >= ammo || bs->ps->ammo[weapon] < 0) );
}
...
float BotDamageRate(bot_state_t *bs, unsigned int weapon_list, combat_zone_t *zone, qboolean splash)
{
	...
	best_rate = 0.0;
	for (weapon = WP_NONE+1, weapon_list >>= weapon;
		 weapon_list;
...
int BotTargetWeapon(bot_state_t *bs)
{
	...
	if ( (bs->weapon != best_weapon) &&
		 (bs->debug_flags & BOT_DEBUG_INFO_WEAPON) )
	{
		BotAI_Print(PRT_MESSAGE,
					"%s: Weapon select: Using %s (%.0f/sec) instead of %s (%.0f/sec)\n",
					EntityNameFast(bs->ent),
					WeaponName(best_weapon), best_damage_rate,
					WeaponName(bs->weapon), old_damage_rate);
	}
#endif
