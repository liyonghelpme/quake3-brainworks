...
int LevelNearestRegions(vec3_t point, int max_neighbors, region_t *neighbors[], float dists[])
{
	...
	num_neighbors = octree_neighbors(point, region_tree, RegionLocation,
	...
	for (from = 0; from < num_regions; from++)
	{
		...
		for (to = 0; to < num_regions; to++)
		{
			...
			region_times[from][to] =
				LevelTravelTime(start->area, start->ent->r.currentOrigin,
								end->area,   end->ent->r.currentOrigin,
								TFL_DEFAULT);
		}
	}
}
...
qboolean LevelRegionIsNeighbor(region_t *region, region_t **neighbors, int num_neighbors)
{
	...
	return (bsearch(&region, neighbors, num_neighbors,
					sizeof(region_t *), CompareVoidList)
				? qtrue : qfalse);
}
...
void LevelRegionSetup(item_cluster_t *clusters, int num_clusters)
{
	...
	for (num_regions = 0;
		 num_regions < num_clusters && num_regions < MAX_REGIONS;
		 num_regions++)
	{
		// Set up the region
	...
	for (from = 0; from < num_regions; from++)
	{
		...
		for (to = 0; to < num_regions; to++)
		{
			...
			for (i = 0; i < num_regions; i++)
			{
				...
				travel_weight = interpolate(from_time, to_time,
											bot_item_path_neighbor_weight.value);
