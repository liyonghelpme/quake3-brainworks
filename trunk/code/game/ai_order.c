...
{
	return (bs->order_message_time &&
			bs->order_message_time < bs->command_time);
}
...
void BotOrderReset(bot_state_t *bs)
{
	...
	switch (bs->order_type)
	{
		...
		case ORDER_HELP:		bs->help_teammate      = NULL;	break;
		case ORDER_ACCOMPANY:	bs->accompany_teammate = NULL;	break;
		case ORDER_ATTACK:		bs->order_enemy        = NULL;	break;

		// Don't actually reset any values-- the bot has no order
		case ORDER_NONE:	return;
	}
...
void BotLeadReset(bot_state_t *bs)
{
	...
	if ( (bs->lead_requester != bs->lead_teammate->s.number) &&
		 (bs->lead_requester >= 0) )
	{
		Bot_InitialChat(bs, "lead_stop", teammate, NULL);
...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match, gentity_t *sender)
{
	...
	if ( trap_BotFindMatch(teammate_name, &teammatematch, MTCONTEXT_TEAMMATE) &&
...
void BotMatch_WhatAreYouDoing(bot_state_t *bs, bot_match_t *match, gentity_t *sender)
{
	...
	switch (bs->order_type)
	{
		...
		default:
		case ORDER_NONE:		message_type = "roaming";				break;
		case ORDER_GETFLAG:		message_type = "capturingflag";			break;
		case ORDER_RETURNFLAG:	message_type = "returningflag";			break;
		case ORDER_HARVEST:		message_type = "harvesting";			break;
		case ORDER_ASSAULT:		message_type = "attackingenemybase";	break;
		case ORDER_CAMP:		message_type = "camping";				break;
		case ORDER_PATROL:		message_type = "patrolling";			break;

		// Orders with a player argument
		case ORDER_ATTACK:		message_type = "killing";
								ent = bs->order_enemy;					break;
		case ORDER_HELP:		message_type = "helping";
								ent = bs->help_teammate;				break;
		case ORDER_ACCOMPANY:	message_type = "accompanying";
								ent = bs->accompany_teammate;			break;

		// Orders with a goal item argument
		case ORDER_ITEM:		message_type = "gettingitem";
								goal = &bs->inspect_goal;				break;
		case ORDER_DEFEND:		message_type = "defending";
								goal = &bs->defend_goal;				break;
	}
...
qboolean BotMatch_Order(bot_state_t *bs, bot_match_t *match, gentity_t *sender)
{
	...
	switch (match->type)
	{
		...
		case MSG_HELP:
		case MSG_ACCOMPANY:			order_match = BotMatch_HelpAccompany;	break;
		case MSG_KILL:				order_match = BotMatch_Kill;			break;
		case MSG_DEFENDKEYAREA:		order_match = BotMatch_DefendKeyArea;	break;
		case MSG_GETITEM:			order_match = BotMatch_GetItem;			break;
		case MSG_CAMP:				order_match = BotMatch_Camp;			break;
		case MSG_PATROL:			order_match = BotMatch_Patrol;			break;
		case MSG_GETFLAG:			order_match = BotMatch_GetFlag;			break;
		case MSG_ATTACKENEMYBASE:	order_match = BotMatch_AttackEnemyBase;	break;
#ifdef MISSIONPACK
		case MSG_HARVEST:			order_match = BotMatch_Harvest;			break;
#endif
		case MSG_RETURNFLAG:		order_match = BotMatch_ReturnFlag;		break;

		// These are other order-related messages
		case MSG_DISMISS:			order_match = BotMatch_Dismiss;			break;
		case MSG_WHATAREYOUDOING:	order_match = BotMatch_WhatAreYouDoing;	break;
		case MSG_LEADTHEWAY:		order_match = BotMatch_LeadTheWay;		break;
		case MSG_WHEREAREYOU:		order_match = BotMatch_WhereAreYou;		break;
		case MSG_SUICIDE:			order_match = BotMatch_Suicide;			break;

		// This message is pointless-- bots rush to bases whenever possible
		case MSG_RUSHBASE:			return qtrue;
