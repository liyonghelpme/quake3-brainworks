	...
	if (BotIsCarrier(bs))
	{
		...
		if (gametype == GT_HARVESTER) {
			Com_sprintf(flagstatus, sizeof(flagstatus),
						(BotTeam(bs) == TEAM_RED ? S_COLOR_RED"%2d" : S_COLOR_BLUE"%2d"),
						bs->ps->generic1);
		} else
#endif
...
void BotUpdateTeammatePreference(bot_state_t *bs, gentity_t *teammate, int mask, int value)
{
	...
	if ( ctftaskpreferences[teammate_num].preference != 0x0000 &&
		 !Q_stricmp(teammate_name, ctftaskpreferences[teammate_num].name) )
	{
...
void BotOrder_Accompany(bot_state_t *bs, gentity_t *client)
{
	...
	if (bs->our_target_flag == bs->ent) {
		...
		Bot_InitialChat(bs, "cmd_accompany",
							 EntityNameFast(client),
							 EntityNameFast(bs->our_target_flag),
							 NULL);
		BotSayTeamOrder(bs, client->s.number);
		...
		default:
			...
			if (num_teammates <= 10)
			{
				...
				while (i < num_teammates)
				{
					...
					{
						case 1:		group_size = 1;	break;	// This shouldn't occur
						case 3: 	group_size = 3;	break;
						default:	group_size = 2;	break;
					}
...
void BotCTFOrders_BothFlagsNotAtBase(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates, .4, qfalse,
						  BotOrder_Accompany, BotOrder_ReturnFlag);
}
...
void BotCTFOrders_OurFlagNotAtBase(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates,
						  (bs->team_strategy & STRATEGY_AGGRESSIVE ? .7 : .5), qtrue,
						  BotOrder_GetFlag, BotOrder_ReturnFlag);
}
...
void BotCTFOrders_EnemyFlagNotAtBase(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates, .4, qfalse,
						  BotOrder_Accompany, BotOrder_Defend);
}
...
void BotCTFOrders_BothFlagsAtBase(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates,
						  (bs->team_strategy & STRATEGY_AGGRESSIVE ? .6 : .3), qtrue,
						  BotOrder_GetFlag, BotOrder_Defend);
}
...
void Bot1FCTFOrders_TeamHasFlag(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates,
						  ((bs->team_strategy & STRATEGY_AGGRESSIVE) ? .8 : .7), qtrue,
						  BotOrder_Accompany, BotOrder_Defend);
}
...
void Bot1FCTFOrders_EnemyHasFlag(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates,
						  ((bs->team_strategy & STRATEGY_AGGRESSIVE) ? .4 : .2), qtrue,
						  BotOrder_GetFlag, BotOrder_Defend);
}
...
void BotObeliskOrders(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates,
						  ((bs->team_strategy & STRATEGY_AGGRESSIVE) ? .7 : .5), qtrue,
						  BotOrder_Assault, BotOrder_Defend);
}
...
void BotHarvesterOrders(bot_state_t *bs, gentity_t **teammates, int num_teammates)
{
	...
	BotAttackDefendOrders(bs, teammates, num_teammates,
						  ((bs->team_strategy & STRATEGY_AGGRESSIVE) ? .7 : .5), qtrue,
						  BotOrder_Harvest, BotOrder_Defend);
}
#endif
...
void BotTeamAI(bot_state_t *bs)
{
	...
	switch (gametype)
	{
		case GT_TEAM:		BotTeamOrders(bs, teammates, num_teammates);		break;
		case GT_CTF:		BotCTFOrders(bs, teammates, num_teammates);			break;
#ifdef MISSIONPACK
		case GT_1FCTF:		Bot1FCTFOrders(bs, teammates, num_teammates);		break;
		case GT_OBELISK:	BotObeliskOrders(bs, teammates, num_teammates);		break;
		case GT_HARVESTER:	BotHarvesterOrders(bs, teammates, num_teammates);	break;
#endif
	}
}
...
void BotMatch_TaskPreference(bot_state_t *bs, bot_match_t *match, gentity_t *sender)
{
	...
	{
		case ST_DEFENDER:	BotSetTeammatePreference(bs, sender, TASKPREF_DEFENDER);	break;
		case ST_ATTACKER:	BotSetTeammatePreference(bs, sender, TASKPREF_ATTACKER);	break;
		case ST_ROAMER:		BotSetTeammatePreference(bs, sender, TASKPREF_ROAMER);		break;
	}
}
...
qboolean BotMatch_Team(bot_state_t *bs, bot_match_t *match, gentity_t *sender)
{
	...
	switch (match->type)
	{
		case MSG_CHECKPOINT:			BotMatch_CheckPoint(bs, match, sender);			return qtrue;
		case MSG_STARTTEAMLEADERSHIP:	BotMatch_StartTeamLeaderShip(bs, match, sender);return qtrue;
		case MSG_STOPTEAMLEADERSHIP:	BotMatch_StopTeamLeaderShip(bs, match, sender);	return qtrue;
		case MSG_WHOISTEAMLAEDER:		BotMatch_WhoIsTeamLeader(bs, match);			return qtrue;
		case MSG_WHATISMYCOMMAND:		BotMatch_WhatIsMyCommand(bs, match);			return qtrue;
		case MSG_ENTERGAME:				BotMatch_EnterGame(bs, match);					return qtrue;
		case MSG_NEWLEADER:				BotMatch_NewLeader(bs, match, sender);			return qtrue;
		case MSG_CTF:					BotMatch_CTF(bs, match);						return qtrue;
		case MSG_TASKPREFERENCE:		BotMatch_TaskPreference(bs, match, sender);		return qtrue;
	}
