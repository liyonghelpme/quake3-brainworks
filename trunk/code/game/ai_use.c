...
void BotUseKamikaze(bot_state_t *bs)
{
	...
	if ( bs->team_carrier &&
	     (DistanceSquared(bs->team_carrier->r.currentOrigin, bs->now.origin)
	     	< Square(KAMIKAZE_DIST)) )
	{
		return;
	}

	if ( bs->enemy_carrier &&
		 (DistanceSquared(bs->enemy_carrier->r.currentOrigin, bs->now.origin)
		 	< Square(KAMIKAZE_DIST)) )
	{
	...
	if ( (gametype == GT_OBELISK) &&
		 (!bs->aim_enemy->client) &&
		 (DistanceSquared(bs->aim_enemy->r.currentOrigin, bs->now.origin) < Square(200)) )
	{
		BotCommandAction(bs, ACTION_USE);
		return;
	}
...
void BotUse(bot_state_t *bs)
{
	...
	if (bs->ps->stats[STAT_HEALTH] < 40 &&
		bs->ps->stats[STAT_HOLDABLE_ITEM] == MODELINDEX_TELEPORTER &&
		!BotIsCarrier(bs))
	{
		BotCommandAction(bs, ACTION_USE);
	}
	else if (bs->ps->stats[STAT_HEALTH] < 60 &&
			 bs->ps->stats[STAT_HOLDABLE_ITEM] == MODELINDEX_MEDKIT)
	{
		BotCommandAction(bs, ACTION_USE);
	}
