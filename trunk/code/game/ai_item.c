...
void PrintCluster(item_cluster_t *cluster, int indent)
{
	...
	G_Printf("%sCluster: Location (%.f, %.f, %.f), Area %i, Value: %f\n", tab,
				...
				cluster->center->ent->r.currentOrigin[2],
				cluster->center->area,
				cluster->value);
	...
	for (item = cluster->start; item; item = item->next_near)
	{
		G_Printf("%s  %s (area %i)%s\n", tab,
				 item->ent->item->pickup_name, item->area,
				 (item == cluster->center ? " (center)" : ""));
	}
}
...
===============
*/
qboolean ClustersAddItem(item_cluster_t *clusters, int *num_clusters, int max_clusters,
						 item_link_t *new_item)
{
	int i;
	item_cluster_t *cluster;
	...
	while (i < *num_clusters)
	{
		...
		for (cluster_item = cluster->start; cluster_item; cluster_item = cluster_item->next_near)
		{
			...
			if (DistanceSquared(cluster_item->ent->r.currentOrigin, new_ent->r.currentOrigin) >
				Square(CLUSTER_RANGE))
				continue;
...
void LevelItemSetup(void)
{
	...
	if (bot_debug_item.integer)
	{
		...
		for (i = 0; i < num_important_items; i++)
		{
			...
			G_Printf("  %s (%.f, %.f, %.f) with additional value %.3f\n",
					 ClusterName(cluster),
					 VectorList(cluster->center->ent->r.currentOrigin),
					 cluster->value);
		}
	...
	mm_setup(&dropped_item_mm,
			 clusters_dropped, sizeof(item_cluster_t), MAX_DROPPED_ITEMS,
			 dropped_item_pages, DROPPED_ITEM_PAGES);

	// Initialize the map of dropped items currently on the level
	map_initialize(&dropped_item_map, dropped_item_entries, DROPPED_ITEM_MAP_CAPACITY,
				   CompareVoid, HashEntity);
